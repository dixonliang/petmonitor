...
def main():
  ...
  #######################
  ###### parameters ############################################
  day_time = 28800 # set how many seconds are to elapse before sending a summary
  ...
  for frame in camera.capture_continuous(
    ...
    for obj in results: # check if Leo has been detected on the screen
      if (labels[obj['class_id']] == 'dog' or labels[obj['class_id']] == 'cat' or labels[obj['class_id']] == 'teddy bear'): # possible classifications for Leo
         ...
         #print(obj['bounding_box']) # print where the dog is located
    ...
    if ((bed_time > 20) and (currently_sleeping == False)): # send message that Leo is sleeping if been in bed for longer than a minute that he is not sleeping
    ...
    if ((total_time_elapsed > day_time/2)): # once half way point during day, release
        ser = Servo(PWM("P0")) 
